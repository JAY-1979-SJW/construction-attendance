/**
 * 현장 참여 신청 PENDING 대량 승인/반려 E2E 검증
 *
 * 검증:
 ...
 *
 * 실행:
 *   npx playwright test e2e/site-join-requests-bulk.spec.ts --config=e2e/playwright.config.ts --project=chromium
 */
...
const SCREENSHOT_DIR = path.join(__dirname, '..', 'logs', 'screenshots', 'site-join-bulk')
...
// ── 목 데이터 ──
function makeReq(overrides: { id: string; status: string; workerName: string }) {
  return {
    id: overrides.id,
    ...
    rejectReason: null,
    note: null,
    worker: { id: `w-${overrides.id}`, name: overrides.workerName, phone: '010-0000-0000', jobTitle: '형틀목수' },
    site: { id: 's1', name: '테스트현장', address: '서울시 강남구' },
  }
}

const REQ_PENDING_1 = makeReq({ id: 'jr-p1', status: 'PENDING', workerName: '근로자A' })
const REQ_PENDING_2 = makeReq({ id: 'jr-p2', status: 'PENDING', workerName: '근로자B' })
const REQ_APPROVED  = makeReq({ id: 'jr-a1', status: 'APPROVED', workerName: '근로자C' })

function mockListResp(items: ReturnType<typeof makeReq>[]) {
  return { success: true, data: items, pagination: { page: 1, limit: 30, total: items.length, totalPages: 1 } }
}

async function interceptBulk(page: Page, delay = 0) {
  await page.route('**/api/admin/site-join-requests/bulk', async (route: Route) => {
...
async function goToPage(page: Page, status = 'PENDING') {
  await page.route(`**/api/admin/site-join-requests?status=${status}`, async (route: Route) => {
...
// 데스크탑 (1280px — 테이블 뷰)
// ─────────────────────────────────────────────────────────
test.describe('site-join-requests 대량 승인/반려 [데스크탑]', () => {
  ...
  test('2건 선택 → 대량 승인 → 선택 해제 + 재조회', async ({ page }) => {
    ...
    let call = 0
    await page.route('**/api/admin/site-join-requests?status=PENDING', async (route: Route) => {
      call++
  ...
  test('2건 선택 → 대량 반려 모달 → 사유 입력 → 반려', async ({ page }) => {
    ...
    // Modal은 custom div — textarea의 부모(.p-5) 안의 반려 버튼 클릭
...
// 모바일 (390px — 카드 뷰)
// ─────────────────────────────────────────────────────────
test.describe('site-join-requests 대량 승인/반려 [모바일]', () => {
