/**
 * 업체 관리자 신청 PENDING 대량 승인/반려 E2E 검증
 *
 * 검증:
 ...
 *
 * 실행:
 *   npx playwright test e2e/company-admin-requests-bulk.spec.ts --config=e2e/playwright.config.ts --project=chromium
 */
...
const SCREENSHOT_DIR = path.join(__dirname, '..', 'logs', 'screenshots', 'company-admin-bulk')
...
// ── 목 데이터 ──
function makeReq(overrides: { id: string; status: string; companyName: string }) {
  return {
    id: overrides.id,
    applicantName: '홍길동',
    phone: '01012345678',
...
const REQ_PENDING_1 = makeReq({ id: 'car-p1', status: 'PENDING', companyName: '테스트업체A' })
const REQ_PENDING_2 = makeReq({ id: 'car-p2', status: 'PENDING', companyName: '테스트업체B' })
const REQ_APPROVED  = makeReq({ id: 'car-a1', status: 'APPROVED', companyName: '승인업체C' })
...
async function interceptBulk(page: Page, delay = 0) {
  await page.route('**/api/admin/company-admin-requests/bulk', async (route: Route) => {
...
async function goToPage(page: Page, status = 'PENDING') {
  await page.route(`**/api/admin/company-admin-requests?status=${status}`, async (route: Route) => {
  ...
  await page.goto(`${BASE}/admin/company-admin-requests`)
...
// 데스크탑 (1280px — 테이블 뷰)
// ─────────────────────────────────────────────────────────
test.describe('company-admin-requests 대량 승인/반려 [데스크탑]', () => {
  ...
  test('2건 선택 → 대량 승인 → 선택 해제 + 재조회', async ({ page }) => {
    ...
    let call = 0
    await page.route('**/api/admin/company-admin-requests?status=PENDING', async (route: Route) => {
      call++
  ...
  test('2건 선택 → 대량 반려 모달 → 사유 입력 → 반려', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder="공통 반려 사유를 입력하세요."]', '서류 미비')
...
// 모바일 (390px — 카드 뷰)
// ─────────────────────────────────────────────────────────
test.describe('company-admin-requests 대량 승인/반려 [모바일]', () => {
