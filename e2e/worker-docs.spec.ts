...
test('D-02 근로계약서 상태 수정 저장 — PATCH /docs 호출', async ({ page }) => {
  ...
  // 서류/교육 섹션의 수정 버튼 클릭 — 기본 정보 섹션 수정과 구분
  const docSection = page.locator('div').filter({ has: page.getByText('서류 / 교육 현황', { exact: true }) }).first()
  await docSection.locator('button').filter({ hasText: '수정' }).click()
...
test('D-04 목록 배지 — contractWrittenYn=true 저장 후 hasContract 반영 확인', async ({ page }) => {
  ...
  await page.route('**/api/admin/workers**', async (route: Route) => {
    if (!route.request().url().includes('/workers/')) {
      await route.fulfill({
        status: 200, contentType: 'application/json',
        body: JSON.stringify({
          success: true,
  ...
  // 목록에서 근로자 표시 확인
  await expect(page.getByText('홍길동', { exact: true })).toBeVisible({ timeout: 10000 })
