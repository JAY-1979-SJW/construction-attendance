...
### F-1. crontab 서버 실제 적용 확인

- **내용**: `deploy/crontab.txt` 파일은 생성됐으나 서버 crontab에 실제 적용됐는지 미확인
- **확인 명령**: `ssh haehan-app "crontab -l"`
- **적용 대상 cron**:
  - `0 4 * * *` — 자동 퇴근 (run-auto-checkout)
  - `0 9 * * 1` (1~7월) / `0 9 * * 1-5` (8~12월) — 보험요율 고시 확인
- **레이어**: 운영 설정
- **우선순위**: 즉시
