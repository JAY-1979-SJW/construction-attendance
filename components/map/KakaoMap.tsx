...
export default function KakaoMap({ lat, lng, markerTitle, height = '280px' }: KakaoMapProps) {
  ...
  if (!JS_KEY) {
    return (
      <div style={{ height }}
        className="flex items-center justify-center rounded-lg border border-white/[0.12] bg-[#1B2838] text-yellow-400 text-sm">
  ...
  if (!hasCoords) {
    return (
      <div style={{ height }}
        className="flex items-center justify-center rounded-lg border border-white/[0.12] bg-[#1B2838] text-[#718096] text-sm">
        위치 정보 없음
      </div>
    )
  }
  ...
  return (
    <div style={{ position: 'relative', height }}>
      {status === 'loading' && (
        <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-[#1B2838] text-[#718096] text-sm z-10">
          지도 불러오는 중...
        </div>
      )}
      {status === 'error' && (
        <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-[#1B2838] text-[#e53935] text-sm z-10">
          지도 로드 실패
        </div>
      )}
      <div
        ...
        className="rounded-lg border border-white/[0.12] overflow-hidden"
      />
    </div>
  )
}
