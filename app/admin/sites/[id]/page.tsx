...
const NOTICE_TYPE_COLORS: Record<string, string> = {
  GENERAL_NOTICE:        'bg-gray-100 text-gray-700',
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center gap-3">
        ...
        <span className="text-gray-400 text-sm">/ 운영 기록</span>
      </div>
      ...
      <div className="px-6 py-6 max-w-5xl mx-auto">
        ...
        {(tab === 'schedules' || tab === 'worklogs' || tab === 'tbm' || tab === 'workers') && (
          <div className="mb-4 flex items-center gap-2">
            ...
            />
            <span className="text-xs text-gray-400">작업일보·TBM·출근자 탭 공용</span>
          </div>
        )}
        ...
        {tab === 'info' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-gray-700">현장 기본정보</h2>
              {!infoEditing && (
                <button
            ...
            {infoLoading ? (
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 space-y-4">
                <h3 className="text-sm font-semibold text-blue-800">기본정보 수정</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="text-xs text-gray-600 block mb-1">현장명 *</label>
                    <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={infoForm.name}
                      onChange={(e) => setInfoForm((f) => ({ ...f, name: e.target.value }))} />
                  </div>
                  <div className="col-span-2">
                    <label className="text-xs text-gray-600 block mb-1">주소</label>
                    <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={infoForm.address}
                      onChange={(e) => setInfoForm((f) => ({ ...f, address: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">위도</label>
                    <input type="number" step="any" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={infoForm.latitude}
                      onChange={(e) => setInfoForm((f) => ({ ...f, latitude: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">경도</label>
                    <input type="number" step="any" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={infoForm.longitude}
                      onChange={(e) => setInfoForm((f) => ({ ...f, longitude: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">인증 반경 (m)</label>
                    <input type="number" min="10" max="5000" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={infoForm.allowedRadius}
                      onChange={(e) => setInfoForm((f) => ({ ...f, allowedRadius: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">현장 코드</label>
                    <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={infoForm.siteCode}
                      onChange={(e) => setInfoForm((f) => ({ ...f, siteCode: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">착공일</label>
                    <input type="date" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={infoForm.openedAt}
                      onChange={(e) => setInfoForm((f) => ({ ...f, openedAt: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">준공일</label>
                    <input type="date" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
              ...
              <div className="bg-white border rounded-xl p-5 space-y-4">
                <div className="grid grid-cols-2 gap-y-3 text-sm">
                  <div>
                    <span className="text-xs text-gray-400 block">현장명</span>
                    <span className="font-medium text-gray-800">{siteInfo.name}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-400 block">현장 코드</span>
                    <span className="text-gray-700">{siteInfo.siteCode ?? '—'}</span>
                  </div>
                  <div className="col-span-2">
                    <span className="text-xs text-gray-400 block">주소</span>
                    <span className="text-gray-700">{siteInfo.address}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-400 block">위도 / 경도</span>
                    <span className="text-gray-700">{siteInfo.latitude}, {siteInfo.longitude}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-400 block">인증 반경</span>
                    <span className="text-gray-700">{siteInfo.allowedRadius}m</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-400 block">착공일</span>
                    <span className="text-gray-700">{fmtDate(siteInfo.openedAt)}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-400 block">준공일</span>
                    <span className="text-gray-700">{fmtDate(siteInfo.closedAt)}</span>
                  </div>
                  <div>
                    <span className="text-xs text-gray-400 block">활성 상태</span>
                    <span className={`text-xs px-2 py-0.5 rounded font-medium ${siteInfo.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {siteInfo.isActive ? '운영중' : '종료'}
                    </span>
                  </div>
                  {siteInfo.notes && (
                    <div className="col-span-2">
                      <span className="text-xs text-gray-400 block">메모</span>
                      <span className="text-gray-700 whitespace-pre-line">{siteInfo.notes}</span>
                    </div>
                  )}
                </div>
              </div>
            ) : (
              <div className="text-center text-gray-400 py-8">정보를 불러올 수 없습니다.</div>
            )}
          </div>
        )}
        ...
        {tab === 'companies' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-gray-700">참여회사 목록</h2>
              <button
            ...
            {/* 참여회사 추가 폼 */}
            {showCompanyForm && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 space-y-4 mb-4">
                <h3 className="text-sm font-semibold text-blue-800">참여회사 추가</h3>

                {/* 모드 선택 */}
                <div className="flex gap-2">
                  <button
                    onClick={() => setCompanyMode('existing')}
                    className={`text-xs px-3 py-1.5 rounded border ${companyMode === 'existing' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300'}`}
                  >
                    기존 회사 연결
                  </button>
                  <button
                    onClick={() => setCompanyMode('new')}
                    className={`text-xs px-3 py-1.5 rounded border ${companyMode === 'new' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300'}`}
                  >
                    신규 회사 등록
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {companyMode === 'existing' ? (
                    <div className="col-span-2">
                      <label className="text-xs text-gray-600 block mb-1">회사 선택 *</label>
                      <select
                    ...
                    <>
                      <div>
                        <label className="text-xs text-gray-600 block mb-1">회사명 *</label>
                        <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      ...
                      <div>
                        <label className="text-xs text-gray-600 block mb-1">사업자등록번호</label>
                        <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      ...
                      <div>
                        <label className="text-xs text-gray-600 block mb-1">대표자</label>
                        <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                          value={newCompanyData.representativeName}
                          onChange={(e) => setNewCompanyData((f) => ({ ...f, representativeName: e.target.value }))} />
                      </div>
                      <div>
                        <label className="text-xs text-gray-600 block mb-1">연락처</label>
                        <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      ...
                      <div>
                        <label className="text-xs text-gray-600 block mb-1">유형</label>
                      ...
                      <div>
                        ...
                        <input type="email" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">계약 유형</label>
                    <select
                      className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">현장 담당자명</label>
                    <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      placeholder="예: 홍길동"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">담당자 연락처</label>
                    <input className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      placeholder="010-0000-0000"
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b">
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">회사명</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">참여 상태</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">인증 상태</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">관리자</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">담당자</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">기간</th>
                      <th className="px-4 py-2.5 text-xs text-gray-600 font-medium text-right">액션</th>
                    </tr>
                  </thead>
                  <tbody>
                    {siteCompanies.map((a) => {
                      ...
                      return (
                        <tr key={a.id} className="border-b hover:bg-gray-50">
                          <td className="px-4 py-2.5">
                            <div className="font-medium text-sm text-gray-800">{a.company.companyName}</div>
                            <div className="text-xs text-gray-400">{ctype[a.contractType] ?? a.contractType}</div>
                          </td>
                          <td className="px-4 py-2.5">
                            <span className={`text-xs px-2 py-0.5 rounded font-medium ${psColor[ps] ?? 'bg-gray-100 text-gray-500'}`}>
                              {psLabel[ps] ?? ps}
                            </span>
                          </td>
                          <td className="px-4 py-2.5">
                            {vs ? (
                              <span className={`text-xs px-2 py-0.5 rounded font-medium ${vsColor[vs] ?? 'bg-gray-100 text-gray-500'}`}>
                                {vsLabel[vs] ?? vs}
                              </span>
                            ) : (
                              <span className="text-xs text-gray-400">내부</span>
                            )}
                          </td>
                          <td className="px-4 py-2.5 text-center text-sm text-gray-600">
                            {a.managerCount > 0 ? (
                              <span className="bg-blue-50 text-blue-700 text-xs px-2 py-0.5 rounded">{a.managerCount}명</span>
                            ) : (
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan={7} className="px-4 py-2 text-xs text-gray-400 bg-gray-50">
        ...
        {tab === 'assigned-workers' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-gray-700">현장 배치 근로자</h2>
              <button
            ...
            {showAssignForm && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 space-y-4 mb-4">
                <h3 className="text-sm font-semibold text-blue-800">근로자 현장 배치</h3>
                <div className="grid grid-cols-2 gap-4">
                  {/* 근로자 검색 */}
                  <div className="col-span-2">
                    <label className="text-xs text-gray-600 block mb-1">근로자 검색 (이름/전화번호) *</label>
                    <input
                      className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                    ...
                    />
                    {wsSearching && <p className="text-xs text-gray-400 mt-1">검색 중...</p>}
                    {workerSearchResults.length > 0 && (
                      <div className="border rounded bg-white shadow-sm mt-1 max-h-40 overflow-y-auto">
                        {workerSearchResults.map((w) => (
                          ...
                          >
                            {w.name} {w.phone && <span className="text-gray-400 text-xs ml-1">{w.phone}</span>}
                          </button>
                        ))}
                      </div>
                    )}
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">소속 회사 ID *</label>
                    <input
                      className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">공종</label>
                    <input
                      className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">배정 시작일 *</label>
                    <input type="date" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                      value={assignForm.assignedFrom}
                      onChange={(e) => setAssignForm((f) => ({ ...f, assignedFrom: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">배정 종료일</label>
                    <input type="date" className="w-full border rounded px-2 py-1.5 text-sm bg-white"
                  ...
                  <div className="col-span-2 flex items-center gap-2">
                    ...
                    <label htmlFor="isPrimary" className="text-sm text-gray-600">주 배정 현장으로 설정</label>
                  </div>
                </div>
                <div className="flex gap-2">
            ...
            {awLoading ? (
              ...
              <div className="bg-white border rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b">
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">이름</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">소속</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">공종</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">배정 기간</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">주 배정</th>
                  ...
                  <tbody>
                    {assignedWorkers.map((a) => (
                      <tr key={a.id} className="border-b hover:bg-gray-50">
                        <td className="px-4 py-2.5 font-medium text-gray-800">{a.worker.name}</td>
                        <td className="px-4 py-2.5 text-gray-500 text-xs">{a.company.companyName}</td>
                        <td className="px-4 py-2.5 text-gray-500 text-xs">{a.tradeType ?? '—'}</td>
                        <td className="px-4 py-2.5 text-gray-500 text-xs">
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan={6} className="px-4 py-2 text-xs text-gray-400 bg-gray-50">
        ...
        {tab === 'notices' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-gray-700">현장 공지</h2>
              <button
                onClick={() => setSNF(true)}
                className="text-sm bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700"
              >
            ...
            {showNoticeForm && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                <h3 className="text-sm font-semibold text-blue-800 mb-3">새 공지 등록</h3>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">공지 유형</label>
                    <select
                      className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">노출 대상</label>
                    <select
                      className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">시작일</label>
                    <input
                      type="date" className="w-full border rounded px-2 py-1.5 text-sm"
                      value={noticeForm.startDate}
                      onChange={(e) => setNoticeForm((f) => ({ ...f, startDate: e.target.value }))}
                    />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">종료일 (선택)</label>
                    <input
                      type="date" className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  <div className="col-span-2">
                    <label className="text-xs text-gray-600 block mb-1">내용</label>
                    <textarea
                      rows={3}
                      className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  <div className="col-span-2 flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="todayHighlight" className="text-sm text-gray-600">오늘 강조 공지</label>
                  </div>
                </div>
                <div className="flex gap-2 mt-3">
                  ...
                  <button
                    ...
                    className="text-sm border px-4 py-1.5 rounded text-gray-600 hover:bg-gray-50"
                  >
                    취소
                  </button>
                </div>
            ...
            {noticeLoading ? (
              ...
              <div className="text-center text-gray-400 py-8">등록된 공지가 없습니다.</div>
            ) : (
              <div className="space-y-3">
                {notices.map((n) => (
                  <div
                    key={n.id}
                    className={`bg-white border rounded-lg p-4 ${n.isTodayHighlight ? 'border-blue-400 shadow-sm' : ''}`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded ${NOTICE_TYPE_COLORS[n.noticeType] ?? 'bg-gray-100 text-gray-600'}`}>
        ...
        {tab === 'schedules' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-gray-700">{fmtDate(selectedDate)} 일정</h2>
              <button
                onClick={() => setSSF(true)}
                className="text-sm bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700"
              >
            ...
            {showSchedForm && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                <h3 className="text-sm font-semibold text-green-800 mb-3">일정 등록 — {selectedDate}</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <label className="text-xs text-gray-600 block mb-1">제목</label>
                    <input
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">일정 유형</label>
                    <select
                      className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">시작 시각 (선택)</label>
                    <input
                      type="datetime-local" className="w-full border rounded px-2 py-1.5 text-sm"
                      value={schedForm.plannedStartAt}
                      onChange={(e) => setSchedForm((f) => ({ ...f, plannedStartAt: e.target.value }))}
                    />
                  </div>
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">종료 시각 (선택)</label>
                    <input
                      type="datetime-local" className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">위치 (선택)</label>
                    <input
                      className="w-full border rounded px-2 py-1.5 text-sm"
                  ...
                  <div className="col-span-2">
                    <label className="text-xs text-gray-600 block mb-1">상세 설명 (선택)</label>
                    <textarea
                      rows={2}
                      className="w-full border rounded px-2 py-1.5 text-sm"
            ...
            {schedLoading ? (
              ...
              <div className="text-center text-gray-400 py-8">등록된 일정이 없습니다.</div>
            ) : (
              <div className="space-y-3">
                {schedules.map((s) => (
        ...
        {tab === 'workers' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-gray-700">{fmtDate(selectedDate)} 출근자 상세</h2>
            ...
            {wrkLoading ? (
              ...
              <div className="text-center text-gray-400 py-8">해당 날짜 인원 데이터가 없습니다.</div>
            ) : (
              <div className="bg-white border rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b">
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">이름</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">팀</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">출근</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">출근시각</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">퇴근시각</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">TBM</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">안전확인</th>
                      <th className="text-left px-4 py-2.5 text-xs text-gray-600 font-medium">특이사항</th>
                    </tr>
                  </thead>
                  <tbody>
                    {workers.map((w) => (
                      <tr key={w.id} className="border-b hover:bg-gray-50">
                        <td className="px-4 py-2.5 font-medium text-gray-800">{w.worker.name}</td>
                        <td className="px-4 py-2.5 text-gray-500">{w.teamLabel ?? '—'}</td>
                        <td className="px-4 py-2.5">
                        ...
                        <td className="px-4 py-2.5 text-gray-600 text-xs">{fmtDateTime(w.checkInAt)}</td>
                        ...
                        <td className="px-4 py-2.5 text-xs text-gray-500">{w.remarks ?? '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
