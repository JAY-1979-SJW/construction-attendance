...
import { FloatingToast, MobileCardList, MobileCard, MobileCardField, MobileCardFields } from '@/components/admin/ui'
      ...
      {tab === 'summary' && (
        <div className="bg-card rounded-[12px] shadow-[0_1px_3px_rgba(0,0,0,0.08)] overflow-hidden">
          {loading ? (
            ...
            <MobileCardList
              items={items}
              keyExtractor={(row) => `${row.workerId}-${row.siteId}`}
              emptyMessage="이 월에 집계된 근무 기록이 없습니다."
              renderCard={(row) => (
                <MobileCard
                  title={<span className="font-semibold text-secondary-brand">{row.workerName}</span>}
                  subtitle={`${row.siteName} · ${row.jobTitle}`}
                  badge={
                    <span
                ...
                >
                  <MobileCardFields>
                    <MobileCardField label="총공수" value={<span className="font-bold">{fmt(row.totalUnits)}</span>} />
                    <MobileCardField label="확정공수" value={<span className="text-secondary-brand">{fmt(row.confirmedUnits)}</span>} />
                    {row.draftUnits > 0 && (
                      <MobileCardField label="집계중" value={<span className="text-accent-hover">{fmt(row.draftUnits)}</span>} />
                    )}
                    <MobileCardField
                      label="단가"
                      value={
                    ...
                    <MobileCardField
                      label="예상노임"
                      value={<span className="font-bold">{row.totalAmount > 0 ? fmtWon(row.totalAmount) : '-'}</span>}
                    />
                  </MobileCardFields>
                </MobileCard>
              )}
        ...
        <div className="bg-card rounded-[12px] shadow-[0_1px_3px_rgba(0,0,0,0.08)] overflow-hidden">
          ...
          {ratesLoading ? (
            ...
            <MobileCardList
              ...
              emptyMessage="이 월에 출퇴근 기록이 있는 근로자가 없습니다."
              renderCard={(item) => (
                <MobileCard
