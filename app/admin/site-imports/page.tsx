...
export default function SiteImportsPage() {
  ...
  return (
    <div className="p-8 min-w-0">
        ...
        {/* 업로드 카드 */}
        <div className="bg-card rounded-[12px] p-5 mb-7 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
          ...
          <div className="bg-brand rounded-lg px-4 py-3 mb-4 text-[12px] text-muted-brand">
            ...
            <code className="block text-secondary-brand">현장명 | 주소 | 허용반경(m) | 현장코드</code>
          ...
          <div className="flex gap-3 items-center flex-wrap">
            ...
            <button onClick={handleUpload} disabled={uploading} className={`px-6 py-[10px] bg-brand-accent text-white border-none rounded-lg cursor-pointer text-[14px] font-semibold ${uploading ? 'opacity-60' : ''}`}>
              {uploading ? '업로드 중 (지오코딩 포함)...' : '업로드 및 파싱'}
            </button>
          </div>

          {uploadMsg && (
            <div className={`mt-3 px-[14px] py-[10px] rounded-lg text-[13px] font-semibold ${uploadMsg.includes('완료') ? 'bg-green-light text-[#2e7d32]' : 'bg-red-light text-[#b71c1c]'}`}>
              {uploadMsg}
            </div>
          )}
