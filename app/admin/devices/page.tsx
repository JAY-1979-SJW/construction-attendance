          ...
          <MobileCardList<DeviceItem>
            ...
            renderCard={(item) => (
              <MobileCard
                ...
                badge={
                  item.isBlocked
                    ? <span className="text-[11px] bg-red-light text-[#c62828] px-2 py-0.5 rounded-[10px] font-bold">차단</span>
                    : <span className="text-[11px] bg-green-light text-[#2e7d32] px-2 py-0.5 rounded-[10px] font-bold">정상</span>
                }
                style={{ background: item.isBlocked ? '#fff8f8' : undefined }}
              >
                <MobileCardFields>
                  <MobileCardField label="플랫폼" value={item.platform ?? '-'} />
                  <MobileCardField label="마지막 로그인" value={fmt(item.lastLoginAt)} />
                </MobileCardFields>
                {canMutate && (
                  <MobileCardActions>
                    {item.isBlocked ? (
                      ...
                      <>
                        <input
                          ...
                          className="px-2 py-1 border border-[rgba(91,164,217,0.3)] rounded text-xs flex-1 min-w-0 bg-transparent text-white"
                        />
                        <button
                          onClick={() => handleBlock(item.id)}
                          disabled={processing === item.id}
                          className="px-3 py-1 bg-[#c62828] text-white border-none rounded cursor-pointer text-xs disabled:opacity-50 shrink-0"
                        >
                          차단
                        </button>
                      </>
