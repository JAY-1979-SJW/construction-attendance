          ...
          <div className="fixed top-0 right-0 h-screen w-[460px] bg-white z-50 flex flex-col shadow-xl">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-[13px]">
                <InfoCell label="고용형태" value={EMP_LABEL[selected.employmentType] || selected.employmentType} />
                <InfoCell label="직종" value={selected.jobTitle} />
                <InfoCell label="작업위치" value={selected.locationDisplayName || '-'} />
                <InfoCell label="반복일수" value={selected.consecutiveDays > 1 ? `${selected.consecutiveDays}일째` : '1일째'} highlight={selected.consecutiveDays > 1} />
              </div>

              {/* 공종/작업사항 */}
              {(selected.tradeFamilyLabel || selected.tradeLabel || selected.taskLabel) && (
                <div className="bg-[#F9FAFB] rounded-lg p-3 text-[13px]">
                  ...
                  {selected.workDetail && (
                    <div className="text-[#6B7280] mt-1 text-[12px]">{selected.workDetail}</div>
                  )}
                </div>
              )}
              ...
              <div className="flex gap-3">
                <ManDayCard label="일 공수" value={Number(selected.todayManDays)} color="#F97316" bg="#FFF7ED" />
                <ManDayCard label="월 공수" value={Number(selected.monthlyManDays)} color="#3B82F6" bg="#F0F9FF" />
                <ManDayCard label="총 공수" value={Number(selected.totalManDays)} color="#6B7280" bg="#F9FAFB" />
              </div>

              {/* 작업시간 */}
              {(selected.workStartTime || selected.workEndTime) && (
                <InfoCell label="작업시간" value={`${selected.workStartTime || ''} ~ ${selected.workEndTime || ''}`} />
              )}

              {/* 작업 내용 */}
              <WorkSection label="어제 작업" text={selected.yesterdayWork} />
              <WorkSection label="금일 작업" text={selected.todayWork} highlight />
              <WorkSection label="명일 작업" text={selected.tomorrowWork} />
