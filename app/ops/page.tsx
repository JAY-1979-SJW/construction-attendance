...
export default function OpsDashboard() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-[22px] font-bold text-fore-brand mb-6">대시보드</h1>
      ...
      {data?.sites.length === 0 ? (
        <div className="text-center px-5 py-[60px] bg-card rounded-lg text-muted-brand">
          <p>배정된 현장이 없습니다.</p>
        ...
        <div className="grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(260px,1fr))]">
          {data?.sites.map(site => (
            <Link
              ...
              className="block bg-card rounded-lg p-5 no-underline shadow-[0_1px_3px_rgba(0,0,0,0.08)] border border-brand transition-shadow duration-150"
            >
              <div className="flex justify-between items-center mb-3">
                <span className="text-[15px] font-semibold text-fore-brand">{site.name}</span>
                <StatusBadge status={site.status} />
              </div>
              <div className="flex flex-col gap-1 text-[13px] text-body-brand">
...
function SummaryCard({ label, value, href, color }: { label: string; value: number; href: string; color: string }) {
  return (
    <Link
      href={href}
      className="flex-[1_1_160px] block bg-card rounded-lg p-5 no-underline shadow-[0_1px_3px_rgba(0,0,0,0.08)]"
      style={{ borderTop: `3px solid ${color}` }}
    >
      <div className="text-[32px] font-bold mb-1.5" style={{ color }}>{value}</div>
      <div className="text-[13px] text-muted-brand">{label}</div>
    </Link>
  )
}
