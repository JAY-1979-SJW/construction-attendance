...
const ACCOUNT_STATUS_MAP: Record<string, { label: string; bg: string; color: string }> = {
  PENDING:  { label: '?πÏù∏ ?ÄÍ∏?, bg: '#fef3c7', color: '#92400e' },
  APPROVED: { label: '?úÏÑ±',     bg: '#d1fae5', color: '#065f46' },
  REJECTED: { label: 'Î∞òÎ†§',     bg: '#fee2e2', color: '#991b1b' },
  SUSPENDED:{ label: 'Ï§ëÏ?',     bg: '#f3f4f6', color: '#6b7280' },
}
  ...
  return (
    <div className="p-8">
      <div className="flex items-center gap-3 mb-5">
        <h1 className="text-[22px] font-bold text-[#111827] m-0">?ëÏóÖ???ÑÌô©</h1>
        <span className="text-[14px] text-[#6b7280] bg-[#f3f4f6] px-[10px] py-[2px] rounded-xl">{total}Î™?/span>
      </div>
        ...
        <div className="bg-white rounded-lg border border-[#e5e7eb] overflow-auto">
          <table className="w-full border-collapse text-[13px]">
            <thead className="bg-[#f9fafb]">
              <tr>
                {['?¥Î¶Ñ', 'ÏßÅÏ¢Ö', 'Í≥†Ïö©?ïÌÉú', 'Î∞∞Ï†ï ?ÑÏû•', '?ÅÌÉú', ''].map(h => (
