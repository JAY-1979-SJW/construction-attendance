...
const TABS: { key: TabKey; label: string }[] = [
  { key: 'workers',    label: '?‘ì—…??ê°€???¹ì¸' },
  { key: 'site-joins', label: '?„ì¥ ì°¸ì—¬ ?¹ì¸' },
]
...
const STATUS_LABEL: Record<string, string> = {
  PENDING:  '?€ê¸?, APPROVED: '?¹ì¸', REJECTED: 'ë°˜ë ¤',
  PENDING_REVIEW: 'ê²€? ì¤‘', ACTIVE: '?œì„±',
}
...
function CompanyApprovalsContent() {
  ...
  return (
    <div className="p-8 max-w-[900px] mx-auto">
      <h1 className="text-[22px] font-bold mb-5">?¹ì¸ ?€ê¸?/h1>
...
export default function CompanyApprovalsPage() {
  return <Suspense fallback={<div className="p-8">ë¡œë”© ì¤?..</div>}><CompanyApprovalsContent /></Suspense>
}
...
function ApprovalTab({ tab }: { tab: TabKey }) {
  ...
  const approve = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setMsg({ type: 'success', text: '?¹ì¸ ì²˜ë¦¬?˜ì—ˆ?µë‹ˆ??' })
        load()
      } else {
        const d = await res.json()
        setMsg({ type: 'error', text: d.message ?? '?¤ë¥˜ ë°œìƒ' })
      }
  ...
  const rejectSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMsg({ type: 'success', text: 'ë°˜ë ¤ ì²˜ë¦¬?˜ì—ˆ?µë‹ˆ??' })
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center text-[#9ca3af] py-12 bg-card border border-[#e5e7eb] rounded-lg">?€ê¸?ì¤‘ì¸ ??ª©???†ìŠµ?ˆë‹¤.</div>
      ) : (
      ...
      {rejectTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-card rounded-[12px] p-6 w-[400px] max-w-[90vw]">
            <h3 className="m-0 mb-3 text-[15px] font-semibold">ë°˜ë ¤ ?¬ìœ </h3>
            <textarea
              rows={4}
              className="w-full border border-[rgba(91,164,217,0.3)] rounded-md p-2 text-[13px] box-border"
              placeholder="ë°˜ë ¤ ?¬ìœ ë¥??…ë ¥?˜ì„¸??
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
            />
            <div className="flex gap-2 mt-3">
              <button onClick={rejectSubmit} className="px-3 py-[5px] bg-[#dc2626] text-white border-0 rounded-[5px] cursor-pointer text-[12px]">ë°˜ë ¤ ?•ì •</button>
              <button onClick={() => setRejectTarget(null)} className="px-3 py-[5px] bg-card text-[#374151] border border-[rgba(91,164,217,0.3)] rounded-[5px] cursor-pointer text-[12px]">ì·¨ì†Œ</button>
            </div>
          </div>
        </div>
      )}
