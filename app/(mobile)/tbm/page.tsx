  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="px-4 pt-4">
        <h2 className="text-[16px] font-bold text-gray-800 mb-1">TBM 일지</h2>
        <p className="text-[12px] text-gray-500 mb-4">오늘의 안전교육(Tool Box Meeting) 내용을 확인하고 참여를 서명합니다.</p>

        {loading ? (
          <div className="text-center py-16 text-sm text-gray-400">불러오는 중...</div>
        ) : records.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-[14px] text-gray-500 mb-1">오늘 등록된 TBM이 없습니다</p>
            <p className="text-[12px] text-gray-400">관리자가 TBM을 등록하면 여기에 표시됩니다.</p>
          </div>
        ) : (
            ...
            {records.map(tbm => (
              <div key={tbm.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <div className="text-[11px] text-gray-400 mb-1">{tbm.siteName}</div>
                    <div className="font-bold text-[15px] text-gray-800">{tbm.title}</div>
                  </div>
                  {tbm.conductedAt && (
                ...
                {tbm.content && (
                  <div className="bg-gray-50 rounded-xl p-4 mb-3 text-[13px] text-gray-700 leading-relaxed whitespace-pre-wrap">
                    {tbm.content}
                  </div>
                )}

                {tbm.notes && (
                  <div className="text-[12px] text-amber-700 bg-amber-50 rounded-lg px-3 py-2 mb-3">
                    {tbm.notes}
                  </div>
                )}
                ...
                {confirmed ? (
                  <div className="flex items-center gap-2 py-3 px-4 bg-green-50 rounded-xl border border-green-200">
                    ...
                    <div>
                      <div className="text-[13px] font-bold text-green-700">TBM 참여 확인 완료</div>
                      {confirmedAt && (
                  ...
                  <button
                    ...
                    disabled={submitting}
                    className="w-full py-3.5 rounded-xl text-[14px] font-bold bg-[#F97316] text-white border-none cursor-pointer disabled:bg-gray-300"
                  >
