...
const schema = z.object({
  ...
  checkOutPhotoId: z.string().optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const openLog = await prisma.attendanceLog.findFirst({
      ...
      select: { siteId: true, id: true, qrToken: true },
    })
    ...
    const isQrSession = !!openLog.qrToken
    await writeAuditLog({
      actorUserId: session.sub,
      actorType: 'WORKER',
      actionType: result.isException ? 'ATTENDANCE_EXCEPTION_CHECK_OUT' : (isQrSession ? 'ATTENDANCE_CHECK_OUT_QR' : 'ATTENDANCE_CHECK_OUT_DIRECT'),
      targetType: 'AttendanceLog',
      targetId: openLog.id,
      summary: result.isException ? `예외 퇴근 — 사유: ${exceptionReason}` : (isQrSession ? 'QR 퇴근' : '직접 퇴근'),
