...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (denyFeature) {
      ...
      if (pc) {
        await logPresenceAudit({ presenceCheckId: pc.id, action: 'PERMISSION_DENIED', actorType: 'ADMIN', actorId: session.sub, message: `${session.role} 역할 confirm 시도 차단 (ATTENDANCE_APPROVE 권한 없음)` })
      }
      return denyFeature
    }
