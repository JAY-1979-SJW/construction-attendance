...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (denyFeature) {
      await logPresenceAudit({ presenceCheckId: params.id, action: 'PERMISSION_DENIED', actorType: 'ADMIN', actorId: session.sub, message: `${session.role} 역할 reject 시도 차단 (ATTENDANCE_APPROVE 권한 없음)` })
      return denyFeature
    }
